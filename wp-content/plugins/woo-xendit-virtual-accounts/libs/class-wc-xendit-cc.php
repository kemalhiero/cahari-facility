...
class WC_Xendit_CC extends WC_Payment_Gateway_CC
{
     ...
     * @var number
     */
    protected $default_minimum_amount = 0;

    /**
     * Maximum amount
     * @var number
     */
    protected $default_maximum_amount = 0;
    ...
    public function __construct()
    {
        ...
        if ($this->testmode) {
            $this->description .= '<br/><br/>' .'<p style="color: red; font-size:80%; margin-top:10px;">'.wp_kses(__('<strong>TEST MODE</strong> - Real payment will not be detected', 'woocommerce-xendit'), ['strong'=>[]]). '</p>' .'<br/><br/>' . sprintf(wp_kses(__('Try card "4000000000000002" with any CVN and future expiration date, or see <a href="%s">Xendit Docs</a> for more test cases', 'woocommerce-xendit'), ['a'=>['href'=>true]]), 'https://dashboard.xendit.co/docs/');
            $this->description = trim($this->description);
        }

        $this->xenditClass = new WC_Xendit_PG_API();

        $this->set_current_currency_min_max_amount(get_woocommerce_currency());
    ...
    /**
     * Set Min/Max amount currency
     *
     * @param $currency
     * @return void
     */
    public function set_current_currency_min_max_amount($currency = 'IDR')
    {
        switch ($currency) {
            case 'IDR':
    ...
    public function payment_scripts()
    {
        ...
        $cc_settings = $this->get_cc_settings();
        $xendit_params['can_use_dynamic_3ds'] = !empty($cc_settings) && $cc_settings['can_use_dynamic_3ds'] ? 1 : 0;
     ...
     * @throws Exception
     */
    public function complete_cc_payment($order, $charge_id, $status, $amount, string $cc_token = "")
    {
        $order_id = $this->is_old_woocommerce_version() ? $order->id : $order->get_id();
        if (!$order->is_paid()) {
            ...
            // Reduce stock levels
            $this->is_old_woocommerce_version() ? $order->reduce_order_stock() : wc_reduce_stock_levels($order_id);
